...
export async function buscarMedicamentosPorPaciente(
  ...
  const resposta = await fetch(
    `${API_URL}/medicamentos?pacienteId=${pacienteId}`,
    {
      headers: {
        'Content-Type': 'application/json',
        ...obterHeadersAutenticacao(),
  ...
  if (!resposta.ok) {
    const corpoErro = await resposta.json().catch(() => null);
    throw new Error(
      corpoErro?.erro ?? 'Não foi possível carregar os medicamentos.'
    );
  }
...
export async function criarAlertaEmergencia(
  ...
  // Consome o corpo da resposta mesmo sem usar o resultado.
